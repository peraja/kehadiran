...
### Diperbaiki
- **Penanganan Validasi dan Persistensi TTE BSrE (Modal & Backend)**:
  - Memperbaiki persistensi status TTE pada [`app/Models/Meeting.php`](file:///Users/abedzul/Desktop/htdocs/rapat/app/Models/Meeting.php) dan [`app/Services/BsreEsignService.php`](file:///Users/abedzul/Desktop/htdocs/rapat/app/Services/BsreEsignService.php) dengan menambahkan kolom TTE (`minutes_signed_at/by/path`, `attendance_signed_at/by/path`, `photos_signed_at/by/path`) ke dalam `$fillable` dan menerapkan `forceFill()->save()` sehingga status dan berkas TTE tersimpan permanen ke database setelah penandatanganan sukses.
...
### Diperbaiki
- **Penyederhanaan Form Modal Penandatangan OPD**:
  - Menghapus tombol *Cek NIP* dan blok umpan balik sinkronisasi pada modal form penandatangan (`opd/settings.blade.php`) sehingga kolom NIP dan NIK tampil lebih ringkas dan bersih sebagai input standar.
...
### Diperbaiki
- **Pengaturan OPD**: Memperbaiki error `Undefined variable $isEditSigner` pada modal form penandatangan di halaman Pengaturan OPD (`resources/views/livewire/opd/settings.blade.php`).

## [1.5.19] - 2026-08-29

### Diubah
- **Dokumentasi**: Memperbaiki deskripsi fitur ekspor dokumen pada berkas README.md agar secara tepat mencantumkan bundel berkas PDF resmi rapat (ZIP).

## [1.5.18] - 2026-08-29

### Diubah
- **Dokumentasi**: Merestrukturisasi dan memadatkan isi berkas README.md agar lebih ringkas, terfokus, dan *to-the-point* dengan instruksi instalasi & deployment cepat.
...
### Diperbaiki
- **Halaman Login**: Memperbaiki error `Undefined constant "showPassword"` yang terjadi karena variabel Alpine.js keliru diproses sebagai konstanta PHP oleh komponen Blade.
