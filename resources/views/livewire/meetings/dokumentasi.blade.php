...
new #[Layout('layouts.app')] class extends Component {
    ...
    public function unlockForRevision(): void
    {
        ...
        $this->meeting->refresh();
        $this->alertKey = hrtime(true);
        $this->successMessage = 'Kunci dokumentasi dibuka untuk revisi.';
    ...
    public function savePhotos()
    {
        ...
        $this->dispatch('meeting-updated');
        $this->alertKey = hrtime(true);
        $this->successMessage = 'Foto berhasil diupload.';
    ...
    public function deletePhoto($id)
    {
        ...
        if ($photo) {
            ...
            $this->dispatch('meeting-updated');
            $this->alertKey = hrtime(true);
            $this->successMessage = 'Foto berhasil dihapus.';
...
<x-meeting-layout :meeting="$meeting" activeTab="dokumentasi">
    ...
    <x-alert type="success" class="mb-5" :wire:key="'dok-alert-'.$alertKey">
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
            ...
            <div class="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2.5 sm:gap-3 w-full sm:w-auto self-start sm:self-auto">
                @if(auth()->user()->hasActiveRole('pimpinan') && !$meeting->photos_signed_at && $meeting->status === 'completed')
                <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'sign-photos-modal'); $wire.openSignModal()" class="only:col-span-2 w-full sm:w-auto inline-flex justify-center items-center px-4 py-2.5 bg-primary-600 hover:bg-primary-700 active:scale-95 text-white rounded-xl font-bold text-xs sm:text-sm transition-all shadow-sm gap-2 cursor-pointer">
                ...
                @if($meeting->status === 'completed')
                    ...
                    @else
                    <a href="{{ route('meetings.export.photos', $meeting->id) }}" target="_blank" class="only:col-span-2 w-full sm:w-auto inline-flex justify-center items-center px-4 py-2.5 bg-white hover:bg-slate-50 border border-slate-300 hover:border-slate-400 active:scale-95 text-slate-700 rounded-xl font-bold text-xs sm:text-sm transition-all shadow-sm gap-2" title="Lihat Pratinjau Dokumentasi">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 sm:p-4 bg-amber-50 border border-amber-200 rounded-2xl text-xs text-amber-800 font-medium">
            <div class="flex items-center gap-2.5">
                ...
                <span class="font-semibold text-amber-900">Dokumentasi dikunci — sudah TTE.</span>
            </div>
            @if($canEdit)
        ...
        <div class="bg-slate-50/70 border border-slate-200/80 p-5 sm:p-6 rounded-2xl shadow-2xs">
            ...
            <form wire:submit="savePhotos" class="space-y-4">
                <div x-data="{
                    ...
                    <div class="relative">
                        <input type="file" wire:model="photos" :key="$uploadKey" id="photos" multiple accept="image/*"
                            ...
                            class="block w-full text-sm text-slate-500 file:mr-3 sm:file:mr-4 file:py-2.5 sm:file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-white file:text-slate-700 hover:file:bg-slate-100 hover:file:text-slate-900 file:shadow-xs file:ring-1 file:ring-slate-300 file:transition-all focus:outline-none cursor-pointer bg-white rounded-xl border border-slate-300 p-1.5" required>
                    </div>
                ...
                <div class="flex justify-end pt-2">
                    <button type="submit" wire:loading.attr="disabled" wire:target="photos, savePhotos" class="w-full sm:w-auto inline-flex items-center justify-center px-6 py-3 sm:py-2.5 bg-primary-600 hover:bg-primary-700 active:scale-95 text-white rounded-xl font-bold text-sm transition-all shadow-sm gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
