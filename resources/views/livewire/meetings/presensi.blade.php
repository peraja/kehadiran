...
new #[Layout('layouts.app')] class extends Component {
    ...
    {
        $query = $this->meeting->attendances()->with('user');

        if (!empty($this->search)) {
            $s = trim($this->search);
            $query->where(function ($q) use ($s) {
                $q->where('guest_name', 'like', "%{$s}%")
                  ->orWhere('guest_nip', 'like', "%{$s}%")
                  ->orWhere('guest_agency', 'like', "%{$s}%")
                  ->orWhere('guest_position', 'like', "%{$s}%")
                  ->orWhereHas('user', function ($uq) use ($s) {
                      $uq->where('name', 'like', "%{$s}%")
                         ->orWhere('nip', 'like', "%{$s}%")
                         ->orWhere('unit_name', 'like', "%{$s}%")
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div class="relative w-full sm:w-72">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama, NIP, atau unit..." class="w-full pl-10 pr-8 py-2 bg-white border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 rounded-xl text-xs font-semibold text-slate-800 placeholder-slate-400 transition-all shadow-xs" />
                @if($search)
        ...
        <!-- Full-Width Attendee Table with Pagination -->
            ...
            <table class="w-full text-left border-collapse min-w-[680px]">
                <thead class="bg-slate-50/80 border-b border-slate-200 text-slate-500">
                    <tr class="text-[11px] font-extrabold uppercase tracking-wider">
                        ...
                        <th class="py-3.5 px-6 text-left">Unit Kerja / Instansi</th>
                    ...
                    <tr>
                        <td colspan="6" class="py-16 px-6 text-center">
                            <div class="flex flex-col items-center justify-center max-w-sm mx-auto">
                                ...
                                <h3 class="text-base font-extrabold text-slate-900">{{ $search ? 'Peserta Tidak Ditemukan' : 'Belum Ada Data Presensi' }}</h3>
                                @if($search)
                                <p class="text-xs text-slate-500 mt-1">Tidak ada hasil yang cocok dengan pencarian "{{ $search }}"</p>
                                <button type="button" wire:click="$set('search', '')" class="mt-3 px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 active:scale-95 text-slate-700 rounded-xl text-xs font-bold transition-all cursor-pointer">
                                    Hapus Pencarian
                                </button>
                                @endif
                            </div>
